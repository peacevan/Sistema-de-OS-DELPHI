...
begin
  ...
  case Application.MessageBox(('Deseja excluir Ordem de Serviço ?') , 'Exclusão de Orderm de Serviço', MB_YESNO + MB_ICONQUESTION) of
    IDYES:
    begin
      //Se sim, deleta o cliente:
      frm_data_module.sql_query_os.Delete;
      ShowMessage('ordem de Serviço excluído com sucesso.');
    end;
    IDNO:
    begin
      exit;
...
procedure Tfrm_ordem_servico.DateTimePicker1Change(Sender: TObject);
begin
 txt_query.Text := formatDateTime('dd/mm/yyyy', DateTimePicker1.Date);
end;
...
begin
  ...
  case rg_options_query.ItemIndex of
    ...
    begin
      ...
      txt_query.Clear;
      lbl_query.Visible := true;
      lbl_query.Caption := 'Digite a descrição da Ordem de Serviço:';
      DateTimePicker1.Visible := false;
     // habilita o botão 'btn_query':
      btn_query.Enabled := true;
    ...
    begin
      txt_query.Visible := false;
      lbl_query.Visible := true;
      lbl_query.Caption := 'Selecione a Data:';
